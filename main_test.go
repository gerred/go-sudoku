...
func TestBoards(t *testing.T) {
	...
	for _, file := range files {
		...
		if satSolver == nil {
			//fmt.Printf("%s: could not solve with SAT\n", file)
		...
		if !board.isSolved() {
			...
			for r := 0; r < 9; r++ {
				for c := 0; c < 9; c++ {
					pos := r*9 + c
					v := board.solved[pos]
					varNum := (r+1)*100 + (c+1)*10 + int(v)
					satSolver.KnownAnswers = append(satSolver.KnownAnswers, sat.SetVar{VarNum: varNum, Value: true})
				}
			}
			satSolver = satSolver.Solve()
			if satSolver == nil {
				fmt.Printf("%s: could not solve with SAT (pass 2)\n", file)
				//t.Fatalf("%s: could not solve with SAT", file)
			}
		}
